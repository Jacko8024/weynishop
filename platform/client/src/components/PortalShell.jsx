 ...
 *
 * Desktop sidebar mode (spec §28 — admin): pass `navGroups`
 * ( [{ title, items: [{to,label,icon,end}] }] ) instead of / in addition to
 * `nav`. On md+ the portal renders a fixed grouped sidebar (Dashboard,
 * Catalog, Finance…, Settings) exactly like the desktop admin spec; the flat
 * `nav` list keeps working for portals that don't opt in. Only existing
 * routes are ever listed — grouping never invents destinations.
 */
export default function PortalShell({
  ...
  // Flatten groups for the mobile drawer / bottom bar (single list).
  const flat = navGroups ? navGroups.flatMap((g) => g.items) : nav;
  ...
  const linkCls = ({ isActive }) =>
    `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition ${isActive ? 'bg-brand-50 text-brand-700' : 'text-slate-700 hover:bg-slate-100'
    }`;

  return (
    <div className="min-h-screen flex flex-col">
      {/* ---------- Desktop grouped sidebar (admin, spec §28) ---------- */}
      {navGroups && (
        <aside className="hidden md:flex fixed top-0 left-0 bottom-0 w-60 flex-col bg-white border-r border-slate-200 z-30">
          <div className="flex items-center gap-2 h-16 px-4 border-b border-slate-200 shrink-0">
            <img src="/logo/weynishopping-icon.png" alt="WeyniShopping" className="w-8 h-8 shrink-0" />
            <span className="font-bold leading-tight">
              {t('brand.name').toUpperCase()}
              <span className="block text-[11px] font-normal text-slate-500">{title}</span>
            </span>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
            {navGroups.map((g) => (
              <div key={g.title}>
                {g.title && (
                  <div className="px-4 pb-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                    {g.title}
                  </div>
                )}
      ...
      {/* Shift the whole page right when the sidebar is fixed (md+ only). */}
      <div className={`min-h-screen flex flex-col flex-1 ${navGroups ? 'md:pl-60' : ''}`}>
        {/* ---------- Header ---------- */}
        <header className="bg-white border-b border-slate-200 sticky top-0 z-30">
          <div className="max-w-7xl mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between gap-2">
            ...
            {/* Inline nav only when NOT in sidebar mode (sidebar already shows it). */}
            {!navGroups && (
              <nav className="hidden md:flex items-center gap-1 flex-wrap">
                {nav.map((n) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `px-3 py-2 rounded-lg text-sm font-medium ${isActive ? 'bg-brand-50 text-brand-700' : 'text-slate-600 hover:bg-slate-100'
                      }`
                    }
                  >
                    {n.label}
            ...
            >
              {nav.map((n) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex flex-col items-center justify-center gap-0.5 py-2 text-[11px] font-medium ${isActive ? 'text-brand-600' : 'text-slate-500'
                    }`
                  }
                >
                  {n.icon}
