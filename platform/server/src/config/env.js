...
export const env = {
  ...
  //
  // Normalization (2026-08-29): values pasted from the Firebase JSON file or
  // passed through Docker/Coolify often arrive double-escaped and/or wrapped
  // in literal quotes, e.g.  \"-----BEGIN...\\n...  (with literal backslash-n
  // pairs and stray escaped quotes). That made the SDK silently refuse to
  // initialize ("looksLikePlaceholder" rejects it), disabling Google sign-in
  // in production. Normalize aggressively:
  //   1. strip wrapping quotes — bare "...' or escaped \"...\"
  //   2. unescape \\\\n (double-escaped) then \\n -> real newline
